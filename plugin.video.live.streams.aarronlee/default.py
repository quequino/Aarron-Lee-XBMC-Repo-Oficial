...
addon = xbmcaddon.Addon('plugin.video.live.streams.aarronlee')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.streams.aarronlee-%s]: %s" %(addon_version, string))
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('LiveStreams-AarronLee','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
