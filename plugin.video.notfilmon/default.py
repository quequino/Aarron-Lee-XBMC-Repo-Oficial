...
def CATEGORIES():
                if ADDON.getSetting('filmon') == 'true':
                        addDir('My Recordings','http://www.filmon.com/my/recordings',5,'http://a3.mzstatic.com/us/r1000/065/Purple/a8/0d/f0/mzl.otgnsovy.png','','My Recordings')
                addDir('Live Now Epg Main Channels','http://www.filmon.com/tvguide/',1,'http://www.filmon.com/tv/themes/filmontv/img/mobile/filmon-logo-stb.png','','TV Guide')
                url='http://www.filmon.com'
                net.set_cookies(cookie_jar)
                html = net.http_GET(url).content
                link = html.encode('ascii', 'ignore')
                match=re.compile('"original_name":"(.+?)".+?"logo_148x148_uri":"(.+?)"',re.DOTALL).findall(link)
                for url, icon in match:
                        iconimage=icon.replace('\/','/')
...
def Channels(url,name,group):
        r=':(.+?),"logo":".+?","big_logo":"(.+?)".+?"title":"(.+?)".+?"alias":"(.+?)"'
        net.set_cookies(cookie_jar)
        html = net.http_GET('http://www.filmon.com').content.encode('ascii', 'ignore')
        link=html.split('{"id"')
        groupname='"%s"'% str(group)
        uniques=[]
        for p in link:
            if groupname in p:
                ...
                name=match[0][2]
                alias=match[0][3]
                url = 'http://www.filmon.com/channel/'+str(alias)
                #print '#########################    '+str(url)
                if name not in uniques:
                    uniques.append(name) 
                    name=name.encode("utf-8")
                    iconimage=icon.replace('\/','/')
                    addDir(name,url,2,iconimage,id,group)
...
def addLink(name,url,iconimage,playPath,app,pageUrl,swfUrl,tcUrl,description):
                ...
                liz.setInfo( type="Video", infoLabels={ "Title": playPath, "Plot": description } )
