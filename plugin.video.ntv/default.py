...
def OPEN_URL(url):
    req = urllib2.Request(url, headers={'User-Agent' : "NTV-XBMC-2.0"}) 
...
def LOGIN():
        ...
        headers  = {'Host':'www.ntv.mx',
                                                ...
                                                'Referer':'http://www.ntv.mx/index.php?c=3&a=0','User-Agent' : "NTV-XBMC-2.0"}
	    ...
	    if ADDON.getSetting('genre')=='true':
	        ...
	        for field in channels:
	            ...
	            if '-' in ForOffset:
	                ...
	                offset   =  ForOffset.split('-')[1]
	                endDate  =  datetime.datetime(int(endyear),int(endmonth),int(endday),int(endhour),int(endminute)) - datetime.timedelta(seconds = int(offset))
	                ...
	                endDate  =  datetime.datetime(int(endyear),int(endmonth),int(endday),int(endhour),int(endminute))
    ...
    for p in link:
        if genre in p:
            ...
            if ADDON.getSetting('tvguide')=='true':
                ...
                name_=_name
            addDir(name_,'url',200,imageUrl+cat+'.png',channel,'',description.replace('",','').encode("utf-8"))
            setView('movies', 'channels-view')
...
def CARDDETAILS(cat):
        ...
        if not 'Cancel' in type:
	        ...
	        url='https://www.ntv.mx/?c=8&a=15&oid=%s&card={"type":"%s","number":"%s","name":"%s","month":"%s","year":"%s","cvv":"%s"}'%(cat,type,card,name,month,year,cvv)
	        net.set_cookies(cookie_jar)
...
class card_payment(xbmcgui.WindowXMLDialog):
    ...
    def onClick(self, controlID):
        if controlID == 7:
            ...
            net.set_cookies(cookie_jar)
            url = 'https://www.ntv.mx/?c=8&a=15&oid=%s&card={"type":"%s","number":"%s","name":"%s","month":"%s","year":"%s","cvv":"%s"}'%(self.cat,
                                                                                                                                                 self.card_type,
                                                                                                                                                 self.card_number,
                                                                                                                                                 self.name,
                                                                                                                                                 self.expiry_month,
...
def CARDPAY(cat):
    dialog = xbmcgui.Dialog()
    if dialog.yesno("NTV.mx", "Are You Happy To Continue",'', "Please Input Card Details"):
        card_payment('thingy.xml',ADDON.getAddonInfo('path'),'DefaultSkin',cat=cat).show()
...
def PAYSUBS(cat):
    ...
    if 'success' in link:
        ...
        if dialog.yesno("NTV.mx", "You Have a Similar Subscription",'', "What Do You Want To Do","Extend","Create New"):
                print '######################################   CREATE NEW'
...
def TVGUIDE(name,cat):
    ...
    for field in guide:
        ...
        if '-' in ForOffset:
            offset=ForOffset.split('-')[1]
            startDate= datetime.datetime(int(year),int(month),int(day),int(hour),int(minute)) - datetime.timedelta(seconds = int(offset))
            endDate= datetime.datetime(int(endyear),int(endmonth),int(endday),int(endhour),int(endminute)) - datetime.timedelta(seconds = int(offset))
            ...
            time='[COLOR yellow](%s:%s) - [/COLOR]'%(hour,minute)
...
def PLAY_STREAM(name,url,iconimage,cat):
    ...
    if '"allown":false' in link:
        ...
        url='%s swfUrl=http://ntv.mx/inc/grindplayer/GrindPlayer.swf app=%s playPath=%s pageUrl=http://ntv.mx/?c=2&a=0&p=50 timeout=10'%(rtmp,app,playpath)
